...
건물과 식생은 240 m 셀로 분리됩니다. 드론 위치를 기준으로 저·중·고 품질에서 각각 약 340/470/620 m 반경의 청크를 활성화하고, 건물은 실제 윤곽에서 단순 박스로, 나무는 줄기·수관에서 저폴리곤 수관으로 전환됩니다. 품질 버튼을 바꾸면 활성 반경과 LOD 거리도 즉시 변경됩니다.
...
- 청크 가시성 스트리밍은 적용됐지만 지역 JSON은 아직 시작 시 한 번에 내려받습니다. 실제 서울 확장 전에는 청크별 데이터 파일과 동적 로더가 필요합니다.

## 다음 개발 단계

다음 우선순위는 OSM·DEM 스냅샷을 청크별 파일로 빌드하고 필요한 청크만 동적으로 가져오는 데이터 로더입니다. 이후 실제 공역 정보와 지역 선택 화면을 붙이면 서울 여러 구역으로 확장할 수 있습니다.
