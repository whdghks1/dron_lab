...
- 한 지역을 정적으로 포함하는 MVP라 공간 청크 스트리밍과 거리별 LOD는 아직 없습니다. 건물과 도로 메시 병합은 적용되어 있습니다.

## 다음 개발 단계

다음 우선순위는 지역 데이터를 공간 청크로 나누고 건물·식생에 거리별 LOD를 적용하는 것입니다. 이후 더 정밀한 서울 DEM/제방 데이터, 실제 공역 정보, 지역 선택 화면을 붙이면 서울 여러 구역으로 확장할 수 있습니다.
