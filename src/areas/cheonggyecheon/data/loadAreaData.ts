...
import { CHEONGGYECHEON_CONFIG } from '../config';
import { CHEONGGYECHEON_VISUALS } from '../visuals';
import { CheonggyecheonChunkSource } from './CheonggyecheonChunkSource';
...
export async function loadCheonggyecheonData(): Promise<LoadedAreaData> {
  await Promise.resolve();
  if (!isSnapshot(snapshot)) throw new Error('청계천 지역 데이터 형식이 올바르지 않습니다.');
  if (!isSnapshot(minimap)) throw new Error('청계천 미니맵 데이터 형식이 올바르지 않습니다.');
  if (!isElevationGrid(elevation)) throw new Error('청계천 표고 데이터 형식이 올바르지 않습니다.');
  return {
    config: CHEONGGYECHEON_CONFIG,
    snapshot,
    minimap,
    elevation,
    chunkSource: new CheonggyecheonChunkSource(),
    visuals: CHEONGGYECHEON_VISUALS,
    airspace: { status: 'unavailable', zones: [] },
  };
}
